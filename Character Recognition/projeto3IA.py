...
class MultiLayersPerceptron():
	def __init__(self, layers, thresholds = True):
		...
		# refere-se a colocação do biases
		for i in range(self.num_layers - 1):
			self.size_layers[i] += 1
	...
	def training(self, training_data, alfa = 0.3, learning_rate = 0.01):
		"""
		...
		while(len(training_data)):
			...
			self.back_propagation(self.num_layers - 1, error, len(training_data), alfa, list_outs, learning_rate)
			training_data.pop()
			tam += 1
			if tam%200 == 0:
				print("\niteração: " + str(tam))
	...
	def back_propagation(self, n_layers, error, quatity_ex, momentum, list_outs, learning_rate):
		...
		while n_layers >= 1:
			...
			#input()
			delta = np.multiply(deriv, error.transpose())
			delta = delta.transpose()  # Vetor coluna

			#input()
			self.deltaW[n_layers-1] = np.dot((learning_rate/quatity_ex), np.dot(delta, np.asarray(list_outs[n_layers-1]).transpose()))

	
			foi = np.dot(momentum, self.deltaW_A[n_layers-1])
			self.deltaW[n_layers-1] = np.add(self.deltaW[n_layers-1], foi)
			nweights = np.add(self.deltaW[n_layers-1] , self.weights[n_layers-1])

			error = np.dot(np.asarray(self.weights[n_layers-1]).transpose(), delta)

			self.weights[n_layers-1] = nweights
			n_layers -=1
